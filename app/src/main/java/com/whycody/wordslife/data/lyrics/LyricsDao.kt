...
interface LyricsDao {
    ...
    @Query("SELECT * FROM lyrics WHERE pl LIKE '%' || :word || '%' LIMIT 1500")
    fun getPlLyricsWithWordIncluded(word: String): List<Lyric>

    @Query("SELECT * FROM lyrics WHERE eng LIKE '%' || :word || '%' LIMIT 1500")
    fun getEngLyricsWithWordIncluded(word: String): List<Lyric>

    @Query("SELECT * FROM lyrics WHERE esp LIKE '%' || :word || '%' LIMIT 1500")
    fun getEspLyricsWithWordIncluded(word: String): List<Lyric>

    @Query("SELECT * FROM lyrics WHERE it LIKE '%' || :word || '%' LIMIT 1500")
    fun getItLyricsWithWordIncluded(word: String): List<Lyric>

    @Query("SELECT * FROM lyrics WHERE ger LIKE '%' || :word || '%' LIMIT 1500")
    fun getGerLyricsWithWordIncluded(word: String): List<Lyric>

    @Query("SELECT * FROM lyrics WHERE fr LIKE '%' || :word || '%' LIMIT 1500")
    fun getFrLyricsWithWordIncluded(word: String): List<Lyric>

    @Query("SELECT * FROM lyrics WHERE pt LIKE '%' || :word || '%' LIMIT 1500")
